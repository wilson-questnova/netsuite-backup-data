...
const FILES_DIR = path.resolve(__dirname, '../files/InvoiceMasterRecord');
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    parser.on('closetag', async (name) => {
      ...
      if (shortName === 'data') {
        ...
        inRow = false;
        if (isHeaderRow) {
          // If the first row contains no headers (or just empty), we should wait for actual headers.
          // But typically the first row is the header row.
          headers = [...currentRow];
          isHeaderRow = false;
        } else {
...
async function run() {
  ...
  console.log(`Found ${files.length} files to process in InvoiceMasterRecord.`);
  ...
  for (const file of files) {
    ...
    try {
      ...
      await parseXmlSpreadsheetStream(path.join(FILES_DIR, file), async (row) => {
        ...
        const status = complete['Status'] ?? 'Billed';
        
        const lineName = complete['Item'] ?? complete['Description'] ?? null;
        const qty = toNumber(complete['Quantity'] ?? 0);
        const amount = toNumber(complete['Amount'] ?? complete['Amount (Gross)'] ?? 0);
