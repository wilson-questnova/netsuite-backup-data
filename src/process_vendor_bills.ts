...
const FILES_DIR = path.resolve(__dirname, '../files/VendorBills');
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    parser.on('opentag', (node) => {
      ...
      if (shortName === 'table') {
        ...
        if (indexAttr) currentCellIndex = parseInt(indexAttr as string, 10) - 1;
    ...
    parser.on('text', (text) => {
      if (inData) currentData += text;
    });

    parser.on('closetag', async (name) => {
      const shortName = (name.toLowerCase().includes(':') ? name.toLowerCase().split(':')[1] : name.toLowerCase());
      if (shortName === 'data') {
        inData = false;
        ...
        if (isHeaderRow) {
          ...
          for (let i = 0; i < headers.length; i++) {
            ...
            if (header) rowObj[header] = currentRow[i] !== undefined ? currentRow[i] : null;
          }
          stream.pause();
          try {
            await onRow(rowObj);
...
async function run() {
  ...
  console.log(`Found ${files.length} files to process in VendorBills.`);
    ...
    try {
      ...
      await parseXmlSpreadsheetStream(path.join(FILES_DIR, file), async (row) => {
        ...
        if (currentId && currentId !== lastId) {
          ...
          context = {
            id: currentId,
            supplier: row['Supplier'] ?? row['Company'] ?? null,
            date: row['Date'] ?? null,
            doc: row['Document Number'] ?? row['Transaction Number'] ?? null,
        ...
        const complete: Record<string, any> = {
          ...
          'Supplier': context.supplier ?? row['Supplier'] ?? row['Company'],
        ...
        const entityName = complete['Supplier'] ?? complete['Company'] ?? null;
        const docNum = complete['Document Number'] ?? null;
        const status = complete['Status'] ?? 'Open';
        const lineName =
          complete['Item'] ??
        ...
        const amount = toNumber(
          complete['Amount'] ??
          complete['Amount (Net)'] ??
          complete['Expense Amount'] ??
          complete['Amount (Transaction Tax Total)'] ??
          complete['Amount (Tax)'] ??
          0
        );
        ...
        inserted++;
        if (inserted % 10000 === 0) console.log(`  - Inserted ${inserted} records...`);
      });
