...
// Item Receipts APIs
app.get('/api/item-receipts', async (req, res) => {
  try {
    const db = await getDb();
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 50;
    ...
    let countQuery = `SELECT COUNT(DISTINCT document_number) as total FROM item_receipts WHERE 1=1`;
    const params: any[] = [];
    ...
    console.error('Item receipts search error:', error);
    res.status(500).json({ error: 'Database error' });
  }
});
...
app.get('/api/vendor-bills', async (req, res) => {
  try {
    ...
    let countQuery = `SELECT COUNT(DISTINCT document_number) as total FROM vendor_bills WHERE 1=1`;
    const params: any[] = [];
    ...
    console.error('Vendor bills search error:', error);
    res.status(500).json({ error: 'Database error' });
  }
});

app.get('/api/vendor-bills-statuses', async (req, res) => {
  try {
    const db = await getDb();
    const rows = await db.all(`
      SELECT DISTINCT status
    ...
    console.error('Vendor bills statuses error:', e);
    res.status(500).json({ error: 'Database error' });
  }
});

app.get('/api/vendor-bills/:docNumber', async (req, res) => {
  const docNumber = (req.params.docNumber || '').trim();
  if (!docNumber) {
    return res.status(400).json({ error: 'Missing document number' });
  }
  try {
    ...
    const lines = rows.map(r => {
      let raw: any = null;
      try { raw = r.raw_data ? JSON.parse(r.raw_data) : null; } catch { raw = null; }
      const item = r.item ?? raw?.['Account (Main)'] ?? raw?.['Applied To Transaction'] ?? '';
      const rate = raw ? (raw['Item Rate'] ?? null) : null;
      return {
        item,
        quantity: r.quantity,
        amount: parseNumber(r.amount),
    ...
    console.error('Vendor bill detail error:', e);
    res.status(500).json({ error: 'Database error' });
  }
});
app.get('/api/item-receipts-statuses', async (req, res) => {
  try {
    const db = await getDb();
    const rows = await db.all(`
      SELECT DISTINCT status
    ...
    console.error('Item receipts statuses error:', e);
    res.status(500).json({ error: 'Database error' });
  }
});
...
app.get('/api/sales-orders', async (req, res) => {
  try {
    ...
    let countQuery = `SELECT COUNT(DISTINCT document_number) as total FROM sales_orders WHERE 1=1`;
    const params: any[] = [];
...
app.get('/api/sales-orders-statuses', async (req, res) => {
  try {
    const db = await getDb();
    const rows = await db.all(`
      SELECT DISTINCT status
    ...
    console.error('Sales orders statuses error:', e);
    res.status(500).json({ error: 'Database error' });
  }
});

app.get('/api/sales-orders/:docNumber', async (req, res) => {
  const docNumber = (req.params.docNumber || '').trim();
  if (!docNumber) {
    return res.status(400).json({ error: 'Missing document number' });
  }
  try {
    ...
    const location = rawHeader?.['Location'] ?? null;
    ...
    const lines = rows.map(r => {
      ...
      return {
        ...
        rate: parseNumber(rate),
        srp: 0,
        discount: raw?.['Transaction Discount'] ?? '',
      };
    });
    ...
    console.error('Sales order detail error:', e);
    res.status(500).json({ error: 'Database error' });
  }
});
app.get('/api/item-receipts/:docNumber', async (req, res) => {
  const docNumber = (req.params.docNumber || '').trim();
  if (!docNumber) {
    return res.status(400).json({ error: 'Missing document number' });
  }
  try {
    ...
    console.error('Item receipt detail error:', e);
    res.status(500).json({ error: 'Database error' });
  }
});
