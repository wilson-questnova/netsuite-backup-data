...
const FILES_DIR = path.resolve(__dirname, '../files/SalesOrder');
...
async function run() {
  ...
  console.log(`Found ${files.length} files to process in SalesOrder.`);
    ...
    try {
      ...
      await parseXmlSpreadsheetStream(path.join(FILES_DIR, file), async (row) => {
        ...
        if (currentId && currentId !== lastId) {
          ...
          context = {
            id: currentId,
            customer: row['Customer Name'] ?? row['Customer : Internal ID'] ?? null,
            date: row['Date'] ?? null,
        ...
        const complete: Record<string, any> = {
          ...
          'Customer Name': context.customer ?? row['Customer Name'],
        ...
        const entityName = complete['Customer Name'] ?? null;
        ...
        const amount = toNumber(
          complete['Amount'] ??
          complete['Amount (Gross)'] ??
          complete['Amount (Tax)'] ??
          complete['Amount Discount'] ??
          0
        );
